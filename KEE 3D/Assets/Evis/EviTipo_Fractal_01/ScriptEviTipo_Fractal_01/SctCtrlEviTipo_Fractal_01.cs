...
//            this.transform.parent.transform.localPosition = new Vector3((escalaAncho / 2f + escalaBoton), (escalaAlto / 2f - escalaBoton), 0f);
    ...
    void Update()
    {
        ...
        if (estOpContenedor == estOpCont_antesDeSolicitarKDL)
        {
            ...
            // Si el contenedor lo es de un efimero, no podemos obtener el KDL de descripcion (ya que los efimeros no estan almacenados en ningun host).
            // en este caso lo que hacemos es generar el evi del concepto, con los datos que tenga la referencia al efimero
...
//        else if (estOpContenedor == estOpCont_recibidoKDL)
        ...
        {
            // antes de 2022-09-30            EviBase.GetComponent<ScriptCtrlBaseDeEvi>().domPropio = KdlConcepto;
            // antes de 2022-09-30            EviBase.GetComponent<ScriptCtrlBaseDeEvi>().nodo_E_ContEnBaseDeEvi = nodo_E_eviFractal;
            // antes de 2022-09-30            EviBase.GetComponent<ScriptCtrlBaseDeEvi>().estado_evi = ScriptCtrlBaseDeEvi.estadoEvi_domRecibido;  // Indicamos al evi base que ya tiene los datos listos
            ...
            {
                EviBase.GetComponent<ScriptCtrlBaseDeEvi>().key = key;
                EviBase.GetComponent<ScriptCtrlBaseDeEvi>().host = host;
                EviBase.GetComponent<ScriptCtrlBaseDeEvi>().cualificador = cualificador;
                EviBase.GetComponent<ScriptCtrlBaseDeEvi>().ordinal = ordinalConf;
                // EviBase.GetComponent<ScriptCtrlBaseDeEvi>().fechUltMod = ultiModConf;
                EviBase.GetComponent<ScriptCtrlBaseDeEvi>().nodo_E_ContEnBaseDeEvi = nodo_E_eviFractal;
                EviBase.GetComponent<ScriptCtrlBaseDeEvi>().estado_evi = ScriptCtrlBaseDeEvi.estadoEvi_domRecibido;  // Indicamos al evi base que ya tiene los datos listos
            }
        ...
        if (estOpContenedor == estOpCont_enCarga)
        {
            ...
            if (tipoDeFractal == ScriptDatosElemenItf.subTipoElemItf_evi_contFractalReferencia)
            {
                ...
                // antes de 2022-09-30            XmlNode KDL_Nodo_C = EviBase.GetComponent<ScriptCtrlBaseDeEvi>().domPropio.DocumentElement; // COmo lo vemos como un get details, la raiz del KDL que hemos obtenido es la raiz del concepto
                // antes de 2022-09-30            ctrlInterfaz.GetComponent<ScriptLibGestorEvis>().desarrollaFractal(EviBase, this.gameObject, KdlConcepto, KDL_Nodo_C);
...
//        else if (estOpContenedor == estOpCont_listo)   //////  OJOOOOO  // debia ser  (estOpContenedor == estOpCont_listo) esto esta pra pruebas hasta que funcione la generacion del fractal
    ...
    ///             3.) Cuando ha llegado el KDL, se guarda en la variable correspondiente y se cambia el estado para que el evi pueda continuar 
