...
//            GetComponent<ScriptLibGestorEvis>().generaEviFractalRef(key_DeEvi, host_DeEvi, cualificador_DeEvi, ordinalConf_DeEvi, ultiModConf_DeEvi, elemDestino_DeEvi);
            ...
            key_DeEvi = "gen_tipoDeSinTecho_FicheroAudio";
            host_DeEvi = ConceptosConocidos.gen_dks_klw_host;
//            GetComponent<ScriptLibGestorEvis>().generaEviFractalRef(key_DeEvi, host_DeEvi, cualificador_DeEvi, ordinalConf_DeEvi, ultiModConf_DeEvi, elemDestino_DeEvi);
...
//            GetComponent<ScriptLibGestorEvis>().generaEviSinTecho_sinDOM(tipoDato_key, tipoDato_host, cualificador_DeEvi, ordinalConf_DeEvi, ultiModConf_DeEvi, elemDestino_DeEvi);
    ...
    IEnumerator cargaDausInterfaz()
    {
        ...
        **** */ 
        ...
        yield return null; // Esto es para pruebas (MAFG 2022-10-15)

        if (DatosGlobal.niveDebug > 50)
        { Debug.Log("Aqui fin de la carga del daus de escena"); }

        // Ahora construimos la interfaz atendiendo al DAUS del USUARIO

        // Configuramos ahora la interfaz, segun corresponda al estado del usuario

            // Actualizamos la lista de idiomas que selecciono el usuario para que tanto la interfaz, como las solicitudes a los DKS, puedan utilizarla
        GetComponent<ScriptGestionaInterfaz>().actualiza_Lista_idiomas_usuario();

        // La interfaz de utiliza conceptos, una vez que tenemos la lista de idiomas, hay que actualizarlos, para que se muestren el la interfaz con
        //  la ayuda a interfaz en el idioma idoneo para el usuario, por qlo que "actualiza_Lista_idiomas_usuario" llama a la funcion 
        // GetComponent<ScriptGestionaInterfaz>().actualiza_conceptos_que_usa_la_interfaz();

        // Habra que ir haciendo mas cosas (PENDIENTE MAFG 2022-10-14)

    } // FIn de - IEnumerator cumplimentaDatosEviTipo00()

}  // Fin de - public class ScriptGestionaEscena : MonoBehaviour {
