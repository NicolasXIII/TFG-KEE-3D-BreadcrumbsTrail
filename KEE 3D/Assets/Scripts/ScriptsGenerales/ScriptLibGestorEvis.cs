...
public class ScriptLibGestorEvis : MonoBehaviour {
    ...
    /// Variables de salida :
    ///         - GameObject objetoContenedor; OJOOO devuelve el contenedor porque el evi base de este evi, no se ha generado todabia, para acceder a el hay que esperar algun frame
    /// Observaciones:
...
//    public void generaEviFractalRef(string key,
    public GameObject generaEviFractalRef(string key,
                                string host,
                                string cualificador,
                                string ordinalConf,
    ...
    /// <summary>
    /// /////////////////////////////////////////////////////////////////
    /// Metodo : generaEviDeEfimero : Genera un evi para una referencia a un concepto efimero y lo coloca en el elemento destino (normalmente un muro) 
    /// Autor : Miguel Angel Fernandez Graciani
    /// Fecha creacion : 2022-09-30
    /// Ultima modificacion :
    /// Variables de entrada :
    ///         - GameObject elemDestino  : es el game object donde debe expandirse la descripcion (normalmente un muro)
    ...
    /// Variables de salida :
    /// Observaciones:
    ///         - Si es UN EFIMERO : No puedo ir a buscarlo al DKS, por lo que tengo que poner la informacion que tenga en el "domPropio"
    ///                             => E (enlace)
    ///                                 => R (REFERENCIA)
    ///                                     => I (identificador)
    ///                                        => k (identificador del efimero)
    ///                                        => H (localizador del efimero (no sera un DKS, si no un KEE o alguien que no lo ha almacenado)
    ///                                        => Q = 1 (es un efimero)
    ///                                     => F (control de configuracion)
    ///                                     => P  (Ayuda a interfaz) (opcional)
    ///                                     => D (solo si el efimero trae descripcion en el KDL de "domPropio")
    ///                                         => E (enlace)
    ///                                         => E (enlace)
    ///                                         .....
    ///                                         => E (enlace)
    ///         - Algunos ejemplos de este tipo de evis de referencia fractal, pueden ser:
    ///             - El evi que aparece en la respuesta de una solicitud al DKS, como referencia a la solicitud que la genero (es necesario para que quien
    ///                 emitio la solicitud y ahora recibe la respuesta, sepa a que solicitud corresponde esta respuesta
    /// </summary>
    public void generaEviDeEfimero(GameObject elemDestino,
                                ...
                                XmlNode nodoEnlace)
    {
        ...
        // es UNA REFERENCIA a un efimero : 
        ...
        XmlNode nodo_Efimero = nodoEnlace.FirstChild; // Decimos que es R, pero lo que nos mandan es un E

        string[] identificadorEfimero_K_H_Q = GetComponent<ScriptLibConceptosXml>().dameIdentificadorDeNodo(domPropio, nodo_Efimero);
        // Asignamos a las variables de este game object
        string efimero_key = identificadorEfimero_K_H_Q[0];                            // key: es el key del concepto (K en KDL - esta en I en KDL)
        string efimero_host = identificadorEfimero_K_H_Q[1];                         // host : es el host del concepto (H en KDL - esta en I en KDL)
        string efimero_cualificador = identificadorEfimero_K_H_Q[2];                 // cualificador : indica la naturaleza del concpto (efimero,...) (Q en KDL - esta en I en KDL)

        string[] ctrlConfInstancia_O_M = GetComponent<ScriptLibConceptosXml>().dameCtrlConfDeNodo(domPropio, nodo_Efimero);
        string efimero_ordinal = ctrlConfInstancia_O_M[0];                         // inst_ordinal : es el ordinal de la instancia (O en KDL - esta en F en KDL)
        string efimero_fechUltMod = ctrlConfInstancia_O_M[1];                 // fecha de ultima modificacion : es fecha de ultima modificacion de la instancia (M en KDL - esta en F en KDL)

        // VERMILISEGUNDOS ponerlos todos en long
        DateTime efimero_fechUltMod_Milisec = new DateTime(0); // Esto esta mal, hay que poner en milisegundos cuando ajustemos
        ...
        {
            Debug.Log("Desde ScriptLibGestorEvis => generaEviDeEfimero. Generamos un evi fractal de efimero." +
            " - Con inst_key = " + efimero_key +
            " - inst_host = " + efimero_host +
            " - inst_cualificador = " + efimero_cualificador +
            " - ordinalConf = " + efimero_ordinal +
            " - ultiModConf = " + efimero_fechUltMod_Milisec +
        ...
        // Cargamos los datos de identificacin del concepto que va a contener, para que pueda proceder a cargarlo
        objetoContenedor.GetComponent<SctCtrlEviTipo_Fractal_01>().key = efimero_key; // Key (KDL:I => K ) del concepto que visualiza
        objetoContenedor.GetComponent<SctCtrlEviTipo_Fractal_01>().host = efimero_host; // host (KDL:I => H ) del concepto que visualiza
        objetoContenedor.GetComponent<SctCtrlEviTipo_Fractal_01>().cualificador = efimero_cualificador; // Cualificador (KDL:I => Q ) del concepto que visualiza, conceptos, efimeros, etc..
        objetoContenedor.GetComponent<SctCtrlEviTipo_Fractal_01>().ordinalConf = efimero_ordinal; // Ordianl (KDL:F=> O) del concepto que visualiza. Ordinal en la secuencia de evolucion (modificaciones) sobre el concepto
        objetoContenedor.GetComponent<SctCtrlEviTipo_Fractal_01>().ultiModConf = efimero_fechUltMod_Milisec; // Ultima modificacion (KDL:F=> M) del concepto que visualiza. Es un instante concreto del tiempo global
        ...
        if (DatosGlobal.niveDebug > 50)
        { Debug.Log("Desde ScriptLibGestorEvis - generaEviDeEfimero con todo anotado = " + nodo_Efimero.Name + " - nodoEnlace = " + nodoEnlace.Name + " - hijo = " + nodo_Efimero.FirstChild.Name); }

        // /////////////////////////////////////////////////////////
        //  2) Marcamos el evi como pendiente de carga para que el mismo evi fractal que hemos generado se encargue de realizar la carga
        //     y construir el evi fractal al completo
        objetoContenedor.GetComponent<ScriptDatosElemenItf>().ponSubTipoElementIntf(ScriptDatosElemenItf.subTipoElemItf_evi_contFractalReferencia); // Indicamos que contiene una instancia

        objetoContenedor.GetComponent<SctCtrlEviTipo_Fractal_01>().estOpContenedor = SctCtrlEviTipo_Fractal_01.estOpCont_recibidoKDL;
//        objetoContenedor.GetComponent<SctCtrlEviTipo_Fractal_01>().estOpContenedor = SctCtrlEviTipo_Fractal_01.estadoEvi_domRecibido;
//        objetoContenedor.transform.parent.gameObject.GetComponent<ScriptCtrlBaseDeEvi>().estado_evi = ScriptCtrlBaseDeEvi.estadoEvi_domRecibido;  // Actualizamos la variable que contiene el valor asociado al elemento T del KDL


        // //////////////////////////////////////
        // Del resto de la carga ya se encarga el evi fractal
    }  // Fin de - generaEviDeEfimero(GameObject elemDestino,
...
//        objetoContenedor.GetComponent<SctCtrlEviTipo_Fractal_01>().nodo_E_eviFractal = nodoEnlace;  // Este es el nodo del enlace que visualiza este evi fractal (debe ser enlace de instancia)
    ...
    /// Ultima modificacion :
    /// Variables de entrada :
    ///     - GameObject ElemenRef : es el elemnto al que hace referencia este evi de referencia
    ///     - GameObject elemDestino : Es el elemento donde hay que haijar (colocar) el elemento de referencia (generalmente la panera, ya que las referencias son migas de pan)
    ///         
    ///     Recordamos
    ///         => E (enlace)
    ///             => R (referencia)
    ...
    public void generaEviRefElemen(GameObject ElemenRef, GameObject elemDestino)
    {
        ...
        EviRefElemen.GetComponent<ScriptDatosElemenItf>().ponSubTipoElementIntf(ScriptDatosElemenItf.subTipoElemItf_evi_EviRefElemen); // Indicamos que es un evi de referencia a elemento de interfaz

        // Asignamos los valores del elemento de interfaz al que hace referencia este "EviRefElemen"
            // Referenciamos gameobget (objeto de la interfaz) al que hacemos referencia
        EviRefElemen.GetComponent<SctCtrlEviRefElemen>().ElemenRef = ElemenRef; 
           // Anotamos en "idElementIntf_referenciado" el identificador unico "idElementIntf" mediante el que podremos localizar el elemento de interfaz referenciado en cualquier momento
        EviRefElemen.GetComponent<SctCtrlEviRefElemen>().idElementIntf_elemenRef = ElemenRef.GetComponent<ScriptDatosElemenItf>().dameIdElementIntf();
        EviRefElemen.GetComponent<SctCtrlEviRefElemen>().tipoElementIntf_elemenRef = ElemenRef.GetComponent<ScriptDatosElemenItf>().dameTipoElementIntf(); // Anotamos el tipo del elemento referenciado
        EviRefElemen.GetComponent<SctCtrlEviRefElemen>().idElementEnTipo_elemenRef = ElemenRef.GetComponent<ScriptDatosElemenItf>().dameIdElementEnTipo(); // Anotamos el id en el tipo del elemento referenciado
        EviRefElemen.GetComponent<SctCtrlEviRefElemen>().subTipoElementIntf_elemenRef = ElemenRef.GetComponent<ScriptDatosElemenItf>().dameSubTipoElementIntf(); // Anotamos el subtipo del elemento referenciado
...
//            if (hijo.transform.parent.gameObject == GetComponent<ScriptDatosInterfaz>().CanvasGeneral)  // Si el elemento es hijo del canvas, hay que controlar su visibilidad
    ...
    *************************** */
    ...
    ///         - Para el caso de HACER REFERENCIA A EVI DE RAMA, el "EviRefElemen" sirve para referenciar esta ramificacion del arbol y poder volver a colocar al usuario en ella cuando se 
    ///             precise. En este caso, cuando el usuario va navegando tomando una de estas bifurcaciones, a cada paso de navegacion, se genera un "EviRefElemen", que, como una miga de pan, 
    ///             se almacena en la panera, para poder seguir la secuencia de pasos de navegación que el usuario va realizando OJOO el evi de rama tambien tiene evibase, por 
    ///             lo que podemos tratarlo de forma muy similar al evi tipo "ScriptDatosElemenItf.tipoElemItf_evi", de hecho es un subtipo de este tipo de evi
    ///         - Para el caso de HACER REFERENCIA A UN MURO, el "EviRefElemen" sirve para referenciar este muro y poder volver a colocar al usuario en el cuando se precise. En este caso,
    ///             cuando el usuario va navegando muro alante, muro atras o realiza saltos de un muro a otro, a cada paso de navegacion, se genera un "EviRefElemen", que, como una miga 
    ///             de pan, se almacena en la panera, para poder seguir la secuencia de pasos de navegación que el usuario va realizando
    ///             
    ///     - Los "EviRefElemen", son elementos de interfaz tipo evi "tipoElemItf_evi" y subtipo "subTipoElemItf_evi_EviRefElemen", por lo que tienen un evi base y el resto de las propiedades
    ///         de los evis  "tipoElemItf_evi". No obstante, hemos decidido que el color de el evi base sera distinto (posiblemente algun marron) para que se distingan como un subtipo 
    ///         distinto, ya que tienen una naturaleza, objeto y funcionalidad muy distinta al de otros subtipos
    ///     
    ///     Pasos de ejecución:
    ...
    ///             2.1.) Si referenciamos a un evi de instancia, referencia, sin techo, rama, etc... osea "tipoElementIntf_elemenRef"="ScriptDatosElemenItf.tipoElemItf_evi"
    ///                 como todos estos elementos de interfaz tienen asociado un evi base, el "EviRefElemen", comparte datos con el evi base, para poder identificar el elemento 
    ///                 de interfaz (tipo "ScriptDatosElemenItf.tipoElemItf_evi") que este "EviRefElemen" referencia
    ///             2.2.) Si referenciamos a una rama "tipoElementIntf_elemenRef"="ScriptDatosElemenItf.tipoElemItf_rama" estos elementos de interfaz no tienen asociado un evi base,
    ///                 OJOO, esto es una rama, no un evi de rama, son cosas distintas (un evi de rama es un elemento tipo evi con subtipo rama. Esto es un elemento tipo rama)
    ///                 el "EviRefElemen", contiene otro tipo de informacion, para poder identificar el elemento de interfaz
    ///             2.3.) Si referenciamos a un muro "tipoElementIntf_elemenRef"="ScriptDatosElemenItf.tipoElemItf_muro" estos elementos de interfaz no tienen asociado un evi base, 
    ///                 el "EviRefElemen", contiene otro tipo de informacion, para poder identificar el elemento de interfaz
    ///             2.4.) Si referenciamos a un agente "tipoElementIntf_elemenRef"="ScriptDatosElemenItf.tipoElemItf_agente" estos elementos de interfaz no tienen asociado un evi base, 
    ///                 el "EviRefElemen", contiene otro tipo de informacion, para poder identificar el elemento de interfaz
    ///         3.) Comppletamos y configuramos la ayuda a interfaz del evi de referencia a elemento de interfaz "EviRefElemen"
    ///         4.) Segundo, atendiendo al contenedor que va aacojer el "EviRefElemen"
    ...
    ///             4.2.) El "EviRefElemen" VA A LA PANERA como una miga de pan
    ///             4.3.) El "EviRefElemen" VA A LA TRAMOYA (por ahora no lo contemplo MAFG 2022-10-13 PENDIENTE o no si no procede)
    ///             4.4.) El "EviRefElemen" VA A UN MURO (por ahora no lo contemplo MAFG 2022-10-13 PENDIENTE o no si no procede)
    /// </summary>
    ...
    IEnumerator esperaYponDatosDeEviBaseDeEviRefElemen(GameObject EviRefElemen, GameObject ElemenRef, GameObject ObjetoPadre)
    {
        ...
        {
            eviParaDatosInterrfaz = ElemenRef;
            ...
            if (EviRefElemen.GetComponent<SctCtrlEviRefElemen>().subTipoElementIntf_elemenRef == ScriptDatosElemenItf.subTipoElemItf_evi_rama)
            {
                ...
                for (int i = 0; i < EviBaseDeEviRefElemen.transform.childCount; i++)
                {
                    ...
                    { EviBaseDeEviRefElemen.transform.GetChild(i).gameObject.GetComponent<MeshRenderer>().material = GetComponent<ScriptDatosInterfaz>().MaterialBtnMaximiza_RefElemItf; }
                    else if (EviBaseDeEviRefElemen.transform.GetChild(i).name == "Btn_BaseDeEvi_N1_Desplazar")
                    { EviBaseDeEviRefElemen.transform.GetChild(i).gameObject.GetComponent<MeshRenderer>().material = GetComponent<ScriptDatosInterfaz>().MaterialBtnDesplazar_RefElemItf; }
                    else if (EviBaseDeEviRefElemen.transform.GetChild(i).name == "Btn_BaseDeEvi_N1_Opciones")
                    { EviBaseDeEviRefElemen.transform.GetChild(i).gameObject.GetComponent<MeshRenderer>().material = GetComponent<ScriptDatosInterfaz>().MaterialBtnOpciones_RefElemItf; }
                }
            ...
            {
                // Le asignamos el DOM y el nodo asociado al concepto o instancia que referencia
                ...
                for (int i = 0; i < EviBaseDeEviRefElemen.transform.childCount; i++)
                {
                    ...
                    {   
                        // En este caso, el evi al que este va a hacer referencia puede estar en navegacion o en edicion.
                            // Si esta en edicion, el boton toma el color de edicion (amarillo)
                        if (eviParaDatosInterrfaz.GetComponent<ScriptDatosElemenItf>().modo == ScriptDatosElemenItf.modoElemItf_edicion)
                        {
                            EviBaseDeEviRefElemen.transform.GetChild(i).gameObject.GetComponent<MeshRenderer>().material = GetComponent<ScriptDatosInterfaz>().MaterialBtnOpcionesEnEdicion;
                        }
                        else  // Si no esta en edicion, ira en el color de navegacion
                        {
            ...
            {
                // OJOO, en principio los evis sin techo no se referencian y creo que en un futuro tampoco van a referenciarse (PENDIENTE MAFG 2022-10-14)
                // Ponemos los botones del evi base, segun proceda
                for (int i = 0; i < EviBaseDeEviRefElemen.transform.childCount; i++)
                {
                    if (EviBaseDeEviRefElemen.transform.GetChild(i).name == "Btn_BaseDeEvi_N1_Maxi_Mini")
        ...
        else if (EviRefElemen.GetComponent<SctCtrlEviRefElemen>().tipoElementIntf_elemenRef == ScriptDatosElemenItf.tipoElemItf_rama)
        {
            ...
            //      OJOO, esto es una rama, no un evi de rama, son cosas distintas (un evi de rama es un elemento tipo evi con subtipo rama. Esto es un elemento tipo rama)
            //     el "EviRefElemen", contiene otro tipo de informacion, para poder identificar el elemento de interfaz
            eviParaDatosInterrfaz = ElemenRef; // PENDIENTE MAFG 2022-10-19 OJOOO hay que cambiarlo por el concepto rama, cuando lo tengamos
            for (int i = 0; i < EviBaseDeEviRefElemen.transform.childCount; i++)
            {
                if (EviBaseDeEviRefElemen.transform.GetChild(i).name == "Btn_BaseDeEvi_N1_Maxi_Mini")
                { EviBaseDeEviRefElemen.transform.GetChild(i).gameObject.GetComponent<MeshRenderer>().material = GetComponent<ScriptDatosInterfaz>().MaterialBtnMaximiza_RefElemItf; }
        ...
        else if (EviRefElemen.GetComponent<SctCtrlEviRefElemen>().tipoElementIntf_elemenRef == ScriptDatosElemenItf.tipoElemItf_muro)
        {
            // 2.3.) Si referenciamos a un muro "tipoElementIntf_elemenRef"="ScriptDatosElemenItf.tipoElemItf_muro" estos elementos de interfaz no tienen asociado un evi base, 
            //     el "EviRefElemen", contiene otro tipo de informacion, para poder identificar el elemento de interfaz
            eviParaDatosInterrfaz = GetComponent<ScriptDatosInterfaz>().gen_muro;
            for (int i = 0; i < EviBaseDeEviRefElemen.transform.childCount; i++)
            {
                if (EviBaseDeEviRefElemen.transform.GetChild(i).name == "Btn_BaseDeEvi_N1_Maxi_Mini")
                { EviBaseDeEviRefElemen.transform.GetChild(i).gameObject.GetComponent<MeshRenderer>().material = GetComponent<ScriptDatosInterfaz>().MaterialBtnMaximiza_RefElemItf; }
        ...
        {
            eviParaDatosInterrfaz = ElemenRef; // PENDIENTE MAFG 2022-10-19 OJOOO hay que cambiarlo por el concepto agente, cuando lo tengamos
        ...
        EviBaseDeEviRefElemen.GetComponent<ScriptCtrlBaseDeEvi>().idioma_AyuIntf = eviParaDatosInterrfaz.GetComponent<ScriptCtrlBaseDeEvi>().idioma_AyuIntf;
        EviBaseDeEviRefElemen.GetComponent<ScriptCtrlBaseDeEvi>().imagen_AyuIntf = eviParaDatosInterrfaz.GetComponent<ScriptCtrlBaseDeEvi>().imagen_AyuIntf;
        EviBaseDeEviRefElemen.GetComponent<ScriptCtrlBaseDeEvi>().icono_AyuIntf = eviParaDatosInterrfaz.GetComponent<ScriptCtrlBaseDeEvi>().icono_AyuIntf;
        EviBaseDeEviRefElemen.GetComponent<ScriptCtrlBaseDeEvi>().audio_AyuIntf = eviParaDatosInterrfaz.GetComponent<ScriptCtrlBaseDeEvi>().audio_AyuIntf;
        EviBaseDeEviRefElemen.GetComponent<ScriptCtrlBaseDeEvi>().txt_nombre_AyuIntf = eviParaDatosInterrfaz.GetComponent<ScriptCtrlBaseDeEvi>().txt_nombre_AyuIntf;
        EviBaseDeEviRefElemen.GetComponent<ScriptCtrlBaseDeEvi>().txt_rotulo_AyuIntf = eviParaDatosInterrfaz.GetComponent<ScriptCtrlBaseDeEvi>().txt_rotulo_AyuIntf;
        EviBaseDeEviRefElemen.GetComponent<ScriptCtrlBaseDeEvi>().txt_descripcion_AyuIntf = eviParaDatosInterrfaz.GetComponent<ScriptCtrlBaseDeEvi>().txt_descripcion_AyuIntf;

        // Cargamos los datos de ayuda a interfaz que van en el canvas del Evi Base, para que actualice los datos del evi base
        EviBaseDeEviRefElemen.GetComponent<ScriptCtrlBaseDeEvi>().textoInfoCanvasEviBase = EviBaseDeEviRefElemen.GetComponent<ScriptCtrlBaseDeEvi>().txt_nombre_AyuIntf;
        string textoInfoCanvasEviBase = EviBaseDeEviRefElemen.GetComponent<ScriptCtrlBaseDeEvi>().txt_nombre_AyuIntf + " : " +
                                        EviBaseDeEviRefElemen.GetComponent<ScriptCtrlBaseDeEvi>().txt_descripcion_AyuIntf;
        EviBaseDeEviRefElemen.GetComponent<ScriptCtrlBaseDeEvi>().caja_texInfoCanvasEviBase.text = textoInfoCanvasEviBase;
        ...
        EviBaseDeEviRefElemen.GetComponent<ScriptCtrlBaseDeEvi>().material_imagen_AyuIntf.SetTexture("_MainTex", eviParaDatosInterrfaz.GetComponent<ScriptCtrlBaseDeEvi>().textura_imagen_AyuIntf);  // Le asinamos la textura al material
                                                                                                                                                                                                     // Ponemos el material en el objeto contenedor, Que es quien contiene la imagen de fondo
        EviBaseDeEviRefElemen.gameObject.GetComponent<ScriptCtrlBaseDeEvi>().ContenedorDeEvi_01.GetComponent<MeshRenderer>().material = eviParaDatosInterrfaz.GetComponent<ScriptCtrlBaseDeEvi>().material_imagen_AyuIntf;
        ...
        EviBaseDeEviRefElemen.GetComponent<ScriptCtrlBaseDeEvi>().textura_ImgAyuIntfBaseDeEvi = eviParaDatosInterrfaz.GetComponent<ScriptCtrlBaseDeEvi>().textura_imagen_AyuIntf; // Utilizamos la misma imagen para el fondo y para la que acompaña al icono
        EviBaseDeEviRefElemen.GetComponent<ScriptCtrlBaseDeEvi>().material_ImgAyuIntfBaseDeEvi.SetTexture("_MainTex", eviParaDatosInterrfaz.GetComponent<ScriptCtrlBaseDeEvi>().textura_ImgAyuIntfBaseDeEvi);  // Le asinamos la textura al material
        EviBaseDeEviRefElemen.gameObject.GetComponent<ScriptCtrlBaseDeEvi>().ImgAyuIntfBaseDeEvi.GetComponent<MeshRenderer>().material = eviParaDatosInterrfaz.GetComponent<ScriptCtrlBaseDeEvi>().material_ImgAyuIntfBaseDeEvi; // Asignamos el material al objeto
        ...
        EviBaseDeEviRefElemen.GetComponent<ScriptCtrlBaseDeEvi>().material_icono_AyuIntf.SetTexture("_MainTex", eviParaDatosInterrfaz.GetComponent<ScriptCtrlBaseDeEvi>().textura_icono_AyuIntf);  // Le asinamos la textura al material
        EviBaseDeEviRefElemen.gameObject.GetComponent<ScriptCtrlBaseDeEvi>().Btn_Evi_Info.GetComponent<MeshRenderer>().material = eviParaDatosInterrfaz.GetComponent<ScriptCtrlBaseDeEvi>().material_icono_AyuIntf; // Asignamos el material al objeto
        ...
        EviBaseDeEviRefElemen.gameObject.GetComponent<ScriptCtrlBaseDeEvi>().Btn_Evi_Info.GetComponent<Script_BaseDeEvi_N1_Info>().audio_AyuIntf = eviParaDatosInterrfaz.GetComponent<ScriptCtrlBaseDeEvi>().AudioClip_AyuIntf;
    ...
    public void gestionaEviRefElementoenLista(GameObject EviBaseDeEviRefElemen, GameObject ObjetoPadre)
    {
        ...
        else if (ObjetoPadre == Tramoya.gameObject)
        {
 //           GameObject ObjetoPadre = this.GetComponent<ScriptDatosInterfaz>().panera;
...
//            EviBaseDeEviRefElemen.transform.localPosition = Tramoya.gameObject.GetComponent<ScriptCtrlTramoya>().damePosicionIniLocal(ObjetoPadre);
        ...
        {
            // 4.3.) El "EviRefElemen" VA A UN MURO (por ahora no lo contemplo MAFG 2022-10-13 PENDIENTE o no si no procede)
            // Para este y otros futuros casos,  habra que programar nuevos "if else". Por ahora lo marcamos como un erros

        }

    } // Fin de - public void gestionaEviRefElementoenLista(GameObject EviBase)
