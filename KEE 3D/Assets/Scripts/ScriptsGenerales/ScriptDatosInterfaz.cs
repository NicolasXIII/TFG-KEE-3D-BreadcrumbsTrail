...
public class ScriptDatosInterfaz : MonoBehaviour {
    ...
    *************** */
    ...
    // Para gestionar el DKS al que se encuentra conectado el KEE en cada momento
    //  - EL KEE se conecta inicialmente al DKS que se indica en "DatosGlobal.Dks_madre_key" y principalmente "DatosGlobal.Dks_madre_host" (que es la url del DKS) 
    //  - No obstante, luego podra conectarse a otros DKSs, mediante un acceso que verifique al usuario correspondiente, (por ahora lo ponemos a piñon PENDIENTE MAFG 2022-09-17)
    //  - OJOOO desde el KEE solo se pueden dar altas o modificaicones de concepto sen un DKS, si previamente nos hemos conectado a el 
    //  - OJO durante el periodo de conexion al DKS habra que asignar estas variables segun porresponda (PENDIENTE MAFG 2022-09-17)
    public string Dks_en_conexion_key;
    public string Dks_en_conexion_host;
    ...
    /// 
    public static string Cualificador_en_DKS = "0";  // Para indicar que un concepto esta almacenado en DKS
    public static string Cualificador_efimero = "1";  // Para indicar que es un concepto efimero, osea, no esta en ningun DKS, no se puede pedir un getdetails
    ...
    // Nicolas Merino Ramirez(2022-10-27)
    ...
    public GameObject almacen_MuroUsuario;  // es un gameobgect que se genera para almacenar algunos objetos que son necesarios para la interfaz, pero que no 
                                            // son visibles al usuario (y si estan como inactivos, no son igual de accesibles). Un ejemplo es el objeto muro, 
                                            // que sirve para usar su ayuda a interfaz en los evis de referencia a elemento de interfaz, enre otros
                                            //  Este gameobject va pegado al muro de  usuario, pero debajo de el, con lo que nunca es visible al usuario (MAFG 2022-10-20)
...
//    public Vector3 distanciaTramoya;
    ...
    // Estos recursos son los asociados a el sistema de miga de pan para ayuda al usuario
    ...
    // Para eol  panera
    public float posicion_x_panera;
    public float posicion_y_panera;
    public float posicion_z_panera;

    public float escala_geneal_panera;  // Define el tamaño general de los botones del muro de usuario. Junto con factorGeneralTamañoObgetos

    public float escala_x_panera;
    public float escala_y_panera;
    public float escala_z_panera;
    public Vector3 posicion_panera;
    public Vector3 escala_panera;

    // almacen_MuroUsuario
    public float posicion_x_almacen_MuroUsuario;
    public float posicion_y_almacen_MuroUsuario;
    public float posicion_z_almacen_MuroUsuario;

    public float escala_geneal_almacen_MuroUsuario;  // Define el tamaño general de los botones del muro de usuario. Junto con factorGeneralTamañoObgetos

    public float escala_x_almacen_MuroUsuario;
    public float escala_y_almacen_MuroUsuario;
    public float escala_z_almacen_MuroUsuario;
    public Vector3 posicion_almacen_MuroUsuario;
    public Vector3 escala_almacen_MuroUsuario;
...
//    public Material MaterialMuroEdicion;
    ...
    public Material MaterialBtnDesplazar_RefElemItf;
    ...
    public Material MaterialBtnMaximiza_RefElemItf;
    ...
    public Material MaterialBtnMinimiza_RefElemItf;
    ...
    public Material MaterialBtnOpciones_RefElemItf;
    ...
    // //////////////////////////////////////////////////////////////////////////
    // //////////////////////////////////////////////////////////////////////////
    // ///  CONCEPTOS EXTERNOS QUE USA LA INTERFAZ

    // Estos conceptos, son conceptos definidos en sus DKSs correspondientes, pero de los cuales la interfaz habla en alguna ocasion
    // por ejemplo, cuando decimos que un evi de referencia refiere un muro, en su ayuda a interfaz debe aparecer informacion asociada
    // al concepto de muro (imagen, icono, nombre, audio,...) para indicar que la referencia lo es a un muro
    // - La ayuda a interfaz podria haberse declarado directamente en el código, pero hemos preferido traer el concepto del DKS cada vez
    // que iniciamos la interfaz por dos razones:
    //      1) para que dicha información este siempre actualizada
    //      2) Para que se muestre en el idioma seleccionado por el usuario. Por lo que estos conceptos deben actualizarse, cada vez que el usuario
    //          modifica la lista de preferencia de idiomas

    public GameObject gen_muro;
    public GameObject gen_listaDeIdiomasDeInterfaz;
...
//        esta_iterfaz_Host = "mi_host_como_interfaz_ideando_net";  // Bebe definirla la entidad que genera esta implantacion (store, fabrica de software, o lo que sea) PENDIENTE MAFG 2022-03-06
        ...
        //  - OJOOO desde el KEE solo se pueden dar altas o modificaicones de concepto sen un DKS, si previamente nos hemos conectado a el 
        //  - OJO durante el periodo de conexion al DKS habra que asignar estas variables segun porresponda (PENDIENTE MAFG 2022-09-17)
        Dks_en_conexion_key = DatosGlobal.Dks_madre_key;
        Dks_en_conexion_host = DatosGlobal.Dks_madre_host;
    ...
    //        ancho_x_Pantalla = 100f;
        ...
        generacion_EviRefElemen_ocupada = false;
...
//        posicion_BtnMenu_N2_Herramientas = new Vector3(((1f / 2f) - (escala_x_BtnMenu_N1_General * 3f / 2f) - (escala_x_BtnMenu_N1_General) * (NumBtn_BtnMenu_N1_1_Gereral_Herramientas / 2f)), -(1f / 2f) + escala_y_BtnMenu_N1_General / 2f, 0f);
        ...
        /// /////////////////    miga de pan - BtnMenu_N2_1_Herramientas_MigaPan ////////////////////////////////////////////////////
        // Nicolas Merino Ramirez(2022-10-27)
        ...
        */
        ...
        escala_y_almacen_MuroUsuario = escalaGeneral_N_1_y; // igual que en x
        escala_z_almacen_MuroUsuario = 1f;  // el muro es un cubo casi plano, solo tiene un poco de profundidad en y, para ajustar los colliders
        escala_BtnMenu_N1_General = new Vector3(escala_x_almacen_MuroUsuario, escala_y_almacen_MuroUsuario, escala_z_almacen_MuroUsuario);
        // Posicion de los botones de nmivel 1 
        posicion_x_almacen_MuroUsuario = (1f / 2f);  // Para que quede en la esquina pero fuera
        posicion_y_almacen_MuroUsuario = (-1f / 2f); // es negativo porque va haci abajo
        posicion_z_almacen_MuroUsuario = 0f;
        posicion_almacen_MuroUsuario = new Vector3(posicion_x_almacen_MuroUsuario, posicion_y_almacen_MuroUsuario, posicion_z_almacen_MuroUsuario);
