...
public class SctExpandirEvi : MonoBehaviour {
    ...
    ///         - 2022-10-12 : Modificaciones para generar las referencias de las migas de pan
    ...
    ///              - Generamos el evi de referencia para la miga de pan
    ///        
    ...
    public void botonExpandeEvi()
    {
        ...
        else
        {
                // ///////////////////////////////////////////
                //  3.1.) Evi subtipo de gestion de rama " subTipoElementIntf = subTipoElemItf_evi_rama" => genera una nueva rama asociada a este evi
                //        SI el evi ES UN EVI DE RAMA, generamos una rama con su muro inicial, para que el usuario tenga la nueva rama de navegacion que ha solicitado
        ...
        if (ListaNodos_E_en_D != null)
        {
            ...
            foreach (XmlNode nodoEnlace in ListaNodos_E_en_D)
            {
                ...
                if (tipoEnlace == ScriptLibConceptosXml.xPathString_name_R)
                {
                    ...
                    if (ref_cualificador != ScriptDatosInterfaz.Cualificador_efimero)
                    {
                        // - Llamamos al generador de evi fractal de referencia  "generaEviFractalRef()"
                    ...
                    } // Fin de - if (ref_cualificador != ScriptDatosInterfaz.Cualificador_efimero)
                    else
                    {
                        // Si es un efimero, no podemos buscar su descripcion en el DKS, por lo que solo podemos ponerle lo que trae en la información del KDL en el 
                        // que se referencia. Es por eso que tenemos que llamar a un generador de evi especifico
                        ctrlInterfaz.GetComponent<ScriptLibGestorEvis>().generaEviDeEfimero(elemDestino,
                                                                                        GetComponent<ScriptCtrlBaseDeEvi>().domPropio,
                                                                                        manejadorEspNomb,
                                                                                        nodoEnlace);
                    } // Fin de - else - de - if (ref_cualificador != ScriptDatosInterfaz.Cualificador_efimero)
...
//            #endif
        ...
        {
            string pathAAbrir = DatosGlobal.Dks_madre_host + "/" + T_DeSinTecho;
            if (DatosGlobal.niveDebug > 50)
            {
        ...
        {
            string pathImgAAbrir = DatosGlobal.Dks_madre_host + "/" + T_DeSinTecho;
            if (DatosGlobal.niveDebug > 50)
            {
            ...
            (this.GetComponent<ScriptCtrlBaseDeEvi>().host == ConceptosConocidos.gen_tipoDeSinTecho_LlamadaFuncionKEE_host))
        {
            ...
            if (T_DeSinTecho == ConceptosConocidos.funcionKEE_ActivadoBoton_BuscadorKee_por_key_host)
            {
                ...
                string hostDeBusqueda = ConceptosConocidos.gen_dks_desarrollo_host;  // esta en el dks de desarrollo
